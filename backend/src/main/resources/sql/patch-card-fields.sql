-- ============================================
-- 银行卡表字段收敛
-- 1. 保留新增/编辑所需字段：用户、银行、尾号、卡类型、信用额度、账单日、还款日、有效期、状态、APP、是否已核实、备注
-- 2. 删除 bank_card 未使用字段：card_no、owner_id、used_amount、owner_relation、owner_name、total_limit
-- 3. APP 字段沿用 repay_method 存储，值限定为 cloudpay/wechat/alipay/other
-- ============================================
...
UPDATE `bank_card` SET `repay_method` = 'other' WHERE `repay_method` NOT IN ('cloudpay', 'wechat', 'alipay', 'other');

ALTER TABLE `bank_card`
  MODIFY COLUMN `repay_method` VARCHAR(20) DEFAULT 'cloudpay' COMMENT 'APP：cloudpay云闪付 wechat微信 alipay支付宝 other其他' AFTER `status`;
...
SET @add_verified = IF(
  ...
  'SELECT ''skip: verified exists'''
);
PREPARE s_add_verified FROM @add_verified; EXECUTE s_add_verified; DEALLOCATE PREPARE s_add_verified;

UPDATE `bank_card` SET `verified` = 0 WHERE `verified` IS NULL;

ALTER TABLE `bank_card`
  MODIFY COLUMN `verified` TINYINT(1) DEFAULT 0 COMMENT '是否已核实' AFTER `repay_method`,
  MODIFY COLUMN `remark` VARCHAR(500) DEFAULT NULL AFTER `verified`;
