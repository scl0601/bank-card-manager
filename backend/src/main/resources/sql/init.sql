...
CREATE TABLE `bank_card` (
  ...
  `balance`         DECIMAL(18,2)   DEFAULT 0.00            COMMENT '当前余额（流水模块维护）',
...
CREATE TABLE `card_bill` (
  ...
  `fee_paid_amount`   DECIMAL(18,2)   DEFAULT 0.00            COMMENT '手续费已支付金额',
  `fee_pay_time`      DATETIME        DEFAULT NULL            COMMENT '最近手续费支付时间',
  `fee_pay_method`    VARCHAR(20)     DEFAULT NULL            COMMENT '手续费支付方式：wechat/alipay/cash/other',
  `verified`          TINYINT(1)      NOT NULL DEFAULT 0      COMMENT '本月还款明细是否已核实',
  `expense_verified`  TINYINT(1)      NOT NULL DEFAULT 0      COMMENT '本月消费明细是否已核实',
  `pos_cost_amount`   DECIMAL(18,2)   DEFAULT 0.00            COMMENT 'POS机使用成本',
  `other_fee_amount`  DECIMAL(18,2)   DEFAULT 0.00            COMMENT '其他费用',
