...
#### 全局视觉风格升级（设计令牌体系重构）
- **全局色彩系统重调** (`styles/index.scss`)
  - 主色调 `--color-primary`: `#1677ff` → `#0958d9`（更深邃的蓝，更专业沉稳）
  - 成功色 `--color-success`: `#52c41a` → `#389e0d`（更沉稳的绿）
  - 警告色 `--color-warning`: `#faad14` → `#d46b08`
  - 危险色 `--color-danger`: `#ff4d4f` → `#cf1322`
  - 信息色 `--color-info`: `#8c8c8c` → `#64748b`
  - 页面背景 `--color-bg`: `#f0f2f5` → `#edf0f7`（更柔和的浅灰蓝）
  - 边框色 `--color-border`: `#e8e8e8` → `#d4d8e2`（分割更清晰）
  - 卡片阴影加深，hover 阴影增强，层次感更好
- **新增文本颜色变量体系**
  - 新增 `--color-text-primary` / `--color-text-regular` / `--color-text-secondary` / `--color-text-placeholder`
  - 全局文字颜色从硬编码 `#333`/`#666`/`#999` 统一迁移为 CSS 变量，便于后续主题切换

#### 日历模块全面增强
- **新增月视图模式** (`CalendarView.vue`)
  - 全新「日/月」视图切换按钮，支持在日视图和月视图之间快速切换
  - 月视图 42 格日历网格（6 行 × 7 列），展示完整当月日期布局
  - 上月/下月导航按钮 + 年份月份快速跳转面板（弹出式选择器）
  - 「今天」快捷回跳按钮
  - 今日高亮、周末标红、法定节假日标注
  - 分类筛选下拉框 + 底部分类颜色图例说明
- **月视图日程展示**
  - 每个格子内显示分类颜色圆点 + 日程标题（最多显示 2~3 条，超出显示"还有 N 项"）
  - 日程数徽标：有日程的格子右上角显示数量角标
  - 单击格子弹出 Popover 弹窗，展示该日期全部日程详情（时间轴 + 标题 + 分类标签 + 状态 + 备注）
  - 双击格子快速创建当日日程
  - 当日进度条：基于待办/进行中/已完成比例可视化
- **搜索功能作用于月视图**
  - 关键词搜索现在同时影响月视图和右侧列表
  - 匹配的日程所在格子以金色边框 + 淡黄色背景高亮显示
  - 搜索与分类筛选可叠加生效（AND 关系），清除后自动恢复
  - 下游联动：日程数徽标、"还有 N 项"、Popover、进度条均基于匹配结果重新计算
- **分类配色统一更新** (`EventDrawer.vue`)
  - 日程分类颜色与全局设计令牌对齐（工作蓝/生活绿/学习橙/健康红/其他灰）
- **月视图统计增强 — 新增「已取消」状态计数**
  - 后端 `CalendarStatsVO` 新增 `cancelledCount` 字段（已取消数量）
  - 后端 `CalendarEventServiceImpl.getMonthStats()` 补充 status=3（已取消）的统计查询
  - 前端月视图统计区域新增「已取消」环形进度展示

#### 首页看板优化 (`DashboardView.vue`)
- 快捷操作区新增「日历计划」入口按钮（Calendar 图标）
- 所有硬编码颜色值替换为 CSS 变量：
  - 统计数值、统计标签、卡片标题 → `--color-text-primary` / `--color-text-secondary`
  - 收支金额正负色 → 与新的成功色/危险色对齐
  - 提醒列表操作者/描述/时间文字 → 统一使用文本色变量
  - hover 背景色调整为与新版背景协调的 `#eef2fa`
- 字体字重微调：关键数字加粗至 700~800，提升数据可读性

### 🐛 Bug 修复
- 修复月视图中统计数据缺少「已取消」状态的问题，现完整展示待办/进行中/已完成/已取消四项统计
